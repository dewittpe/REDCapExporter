...
#'
#' There are `as.data.frame` and `as.data.table` methods for translating the raw
#' export to the noted class.
#'
...
#' can use `format_record`.  The easiest option is to pass the record and
#' metadata to `format_record` as shown below.
#'
temp1 <- format_record(record = avs_record, metadata = avs_raw_metadata)
str(temp1[1:10])

#'
#' The mode for each column is defined by the function call `col_type`. The
#' default modes are set via the metadata:
#'
#' | Field Type                                                             | mode             |
#' | :----------                                                            | :----            |
...
#' input format.
#'
#' * Multiple Choice fields can be set as `character` but passing `factor =
#' FALSE` to `col_type`.
#'
#' * Additional defaults are to be defined.
#'
#' If the user wishes to define the storage mode for a specific subset of
#' columns we recommend the user call `col_type` explicitly, store the result,
#' modify as needed, then pass the modified object to `format_record`.  For
#' example, we will set `hof` which is yes/no variable to `boolean`, and modify
...
read.dcf(paste(prj_path, "DESCRIPTION", sep = "/")) %>% t

#'
#' The package name is as noted above, the version number is worth noting, the
#' version is the year.month.day.hour.minute for when the data was exported from
#' REDCap.
#'
#'
check <- devtools::check(pkg = prj_path, quiet = TRUE)
check

#'
#' We see that the package passes basic checks and can be installed.
#' Let's install the package and explore the contents.

install.packages(grep("*.tar.gz", list.files(path = prj_path, recursive = TRUE, full.names = TRUE), value = TRUE),
                 lib = temppath)
...
#'
#' The data sets provided on this initial export are:
#'
#' * `metadata`: project metadata as a `data.frame`; all columns are characters.
#'
#' * `project`: project info, a `data.frame`; all columns are characters.
#'
#' * `record`: a `data.frame` based on the defualt `format_record` call.
#'
#' * `user`: a `data.frame` of the user data for the project.

data(metadata, record)
all.equal(target  = as.data.frame(avs_metadata),
          current = metadata,
          check.attributes = FALSE)
all.equal(target = format_record(avs_record, metadata),
          current = record,
          check.attributes = FALSE)
